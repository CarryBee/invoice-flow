...
module.exports = class Loop {
    ...
    if (typeof path === 'string') {
      const dip = async function (ctx, next) {
        if (ctx.req && ctx.req.path && ctx.req.path == path) {
          return await Promise.resolve(fn(ctx, await next())); // return 值会去到上一级的 await next();
        } else {
          return await next();
        }
      }
  ...
  run(ctx) {
    ...
    if (Array.isArray(ctx)) {
      const zfns = [];
      const groupfn = compose(this.fns); // 生成 use function 整组
      for (let one of ctx) {
        if (!one || !one.invoices) throw new TypeError("ctx need invoices");
        zfns.push(this.transfn(one, groupfn)); // 每个组都 push
      }
